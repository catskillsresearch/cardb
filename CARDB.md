...
Companion files: `CARDB.lean`, `CARDB/SmallN.lean`, `CARDB/Asymptotics.lean`.
...
| **3** | $29$ | Fiber totals by $|\mathcal{T}|$: $2+12+24+24+48+32$ (the discrete fiber is $2^{8-3}=32$) | **$142$** |

These four values are `card_valid_bases_small` in `CARDB/SmallN.lean`.
...
While no elementary exact formula is known, asymptotic analysis reveals that the sum is doubly exponential and overwhelmingly dominated by a single fiber: the **discrete topology** $\mathcal{T}_{\text{disc}} = \mathcal{P}(S)$.
